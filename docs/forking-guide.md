...
Forking should be easy for most recent traaitt forks that work with turtle-service.
...
```go
var addressFormat = "^XTE([a-zA-Z0-9]{95}|[a-zA-Z0-9]{183})$"
...
There are a few places you may need to make frontend changes for now  Here are some of them, just do a search for `XTE` or `Turtle` to find any I missed.

*services/main/templates/index.html*
```html
<span class="tagline">A secure, easy-to-use wallet for traaitt payments</span>
```
...
<tr>
  <th>Available</th>
  <td><span id="available_balance">{{ printf "%.2f" (index .Wallet "balance" "availableBalance") }} XTE</span></td>
</tr>
<tr>
  <th>Locked / Unconfirmed</th>
  <td><span id="locked_amount">{{ printf "%.2f" (index .Wallet "balance" "lockedAmount") }} XTE</span></td>
</tr>
...
```
```html
<div class="table-container">
    <form action={{ printf "%s%s" .PageAttr.URI "/account/send_transaction"}} method="POST">
        <div class="input-field grey-input">
            <h2>Send Transaction</h2><small>fee: 0.1 XTE</small><br>
            <span class="caret-icon"></span>
            <input id="send_to" type="text" name="destination" placeholder="Enter destination address..." pattern="^XTE([a-zA-Z0-9]{95}|[a-zA-Z0-9]{183})\s*$" required/>
...
<div class="container tx">
 ...
<td><b>Amount</b><br>{{ index $ele "Amount" }}&nbsp;XTE</td>
{{ else }}
